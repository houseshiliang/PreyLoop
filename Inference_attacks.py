...
def test_meminf(device, num_classes, target_train, target_test, shadow_train, shadow_test, dataset_name, 
                    target_model, shadow_model, model_name, train_shadow, use_DP, DP_type, noise, norm, delta,backdoor):
    ...
    if backdoor=="none":
        ...
        if not use_DP:
            ...
            target_path="./data/results/trained_model/target_" + dataset_name + "_" + model_name +"_" + backdoor +  "_" + DP_type + "_" + str(noise) + ".pth"
...
def test_modinv(device, num_classes, target_train, target_model, dataset_name, model_name, use_DP, DP_type, noise, norm, delta, backdoor):
    if backdoor=="none":
        if not use_DP:
            target_path="./data/results/trained_model/target_" + dataset_name + "_" + model_name + ".pth"
            # attack_path="./data/results/inference_attacks/"+ dataset_name + "_" + model_name
...
def test_attrinf(device, num_classes, data_train, data_test, dataset_name, target_model, model_name, use_DP, DP_type, noise, norm, delta, backdoor):
    if backdoor=="none":
        if not use_DP:
            target_path="./data/results/trained_model/target_" + dataset_name + "_" + model_name + ".pth"
            attack_path="./data/results/inference_attacks/"+ dataset_name + "_" + model_name
    ...
    for inputs, [fa, targets] in attack_testloader:
        print(fa, targets)
        return
    image_size = [1] + list(data_train[0][0].shape)
    print("image_size: ", image_size)
    # print(data_train[0][0])
    attrinf_res_train, attrinf_res_test=train_attack_model(target_path, attack_path, num_classes, device, target_model, 
...
def main():
    ...
    parser.add_argument('-ne', '--noise', type=float, default=1.3)  # sigma σ = noise
    ...
    meminf_res_train3, meminf_res_test3, meminf_res_train0, meminf_res_test0=test_meminf(device, num_classes, 
                ...
                model_name, train_shadow, use_DP, DP_type, noise, norm, delta, backdoor)
    ...
    modinv_res_ccs, modinv_res_revealer=test_modinv(device, num_classes, target_train, target_model, dataset_name, 
                                                        model_name, use_DP, DP_type, noise, norm, delta, backdoor)
    ...
    if dataset_name=="utkface":
        ...
        attr="race_gender".split("_")    #  ['race', 'gender']
        # prepare dataset
        num_classes, target_train, target_test, shadow_train, shadow_test, target_model, shadow_model = prepare_dataset(
            dataset_name, attr, root, model_name)
        # print (num_classes)    #  [4, 2]

        attrinf_res_train, attrinf_res_test=test_attrinf(device, num_classes, shadow_train, target_train, dataset_name, 
                                                            target_model, model_name, use_DP, DP_type, noise, norm, delta, backdoor)
    ...
    #                                             header=False)
    #     else:
    #         pd.DataFrame(train_results).to_csv("./data/results/inference_final_results/train_results_"+dataset_name+"_"+model_name+ "_" + DP_type + "_" + str(noise) +".csv", index=False,
    #                                             header=False)
    #         pd.DataFrame(test_results).to_csv("./data/results/inference_final_results/test_results_"+dataset_name+"_"+model_name+ "_" + DP_type + "_" + str(noise) +".csv", index=False,
    #                                             header=False)
    # else:
    #     if not use_DP:
    ...
    #                                             header=False)
    #     else:
    #         pd.DataFrame(train_results).to_csv("./data/results/inference_final_results/train_results_"+dataset_name+"_"+model_name+"_"+backdoor+ "_" + DP_type + "_" + str(noise) +".csv", index=False,
    #                                             header=False)
    #         pd.DataFrame(test_results).to_csv("./data/results/inference_final_results/test_results_"+dataset_name+"_"+model_name+"_"+backdoor+ "_" + DP_type + "_" + str(noise) +".csv", index=False,
    #                                             header=False)
