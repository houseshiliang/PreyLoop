...
class attack_training():
    def __init__(self, device, use_DP, attack_trainloader, attack_testloader, target_model, TARGET_PATH, ATTACK_PATH):
        self.device = device
    ...
    def init_attack_model(self, size, output_classes):
        ...
        self.attack_model = attrinf_attack_model(inputs=input_classes, outputs=output_classes)  # 512  [4, 2]
    ...
    def get_middle_output(self, x):
        ...
        # print("--------get_middle_output-input_classes-----: ", out[name[1]].flatten().shape[0])
        return out[name[1]]
    ...
    def train(self, epoch):
        ...
        for batch_idx, (inputs, [_, targets]) in enumerate(self.attack_trainloader):
            # print("----shape-----",inputs.shape,_.shape,targets.shape)
            # print(_)
            # print(targets)
            # print(inputs[0])
            ...
            # print("----oracles.shape----: ", oracles.shape)
...
def train_attack_model(TARGET_PATH, ATTACK_PATH, output_classes, device, use_DP, target_model, train_loader, test_loader, size):
    attack = attack_training(device, use_DP, train_loader, test_loader, target_model, TARGET_PATH, ATTACK_PATH)

    # print("-------size--------: ", size)    #  [1, 3, 64, 64]
    # print("-------output_classes--------: ", output_classes)    #  4

    attack.init_attack_model(size, output_classes[1])

    for epoch in range(50):
        flag = 1 if epoch==49 else 0
