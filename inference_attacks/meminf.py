...
from inference_attacks.Metrix_class import Metric
...
class attack_for_blackbox():
    def __init__(self, SHADOW_PATH, TARGET_PATH, ATTACK_SETS, attack_train_loader, attack_test_loader, 
                    target_model, shadow_model, attack_model, device, use_DP):
        self.device = device
        ...
        if use_DP:
            if not ModuleValidator.is_valid(target_model):
        ...
        # self.optimizer = optim.SGD(self.attack_model.parameters(), lr=1e-5, momentum=0.9, weight_decay=5e-4)
        ...
        with open(self.ATTACK_SETS + "train.p", "rb") as f:
            while(True):
                try:
                    ...
                    # train_x=np.concatenate((output.cpu().detach().numpy(),prediction.cpu().detach().numpy()),axis=1)
                    ...
                    # rf = clf.fit(train_x, members.cpu().detach().numpy())
            ...
            with open(self.ATTACK_SETS + "test.p", "rb") as f:
                while(True):
                    try:
                        ...
                        # # *******************************************************************
                        # prediction=prediction.reshape([-1,1])
                        # test_x=np.concatenate((output.cpu().detach().numpy(),prediction.cpu().detach().numpy()),axis=1)
                        # predict_y = rf.predict(test_x)
                        # demo = Metric(members.cpu().detach().numpy(), predict_y)
                        # acc = demo.Accuracy()
                        # return acc
                        # # print("--------output--------: ",output.shape)
                        # # print("--------prediction--------: ",prediction.shape,prediction[0])
...
class attack_for_whitebox():
    def __init__(self, TARGET_PATH, SHADOW_PATH, ATTACK_SETS, attack_train_loader, attack_test_loader, 
                    target_model, shadow_model, attack_model, device, use_DP, class_num):
        self.device = device
    ...
    def prepare_dataset(self):
        with open(self.ATTACK_SETS + "train.p", "wb") as f:
            print(self.ATTACK_SETS+"train.p", len(self.attack_train_loader))
            for inputs, targets, members in self.attack_train_loader:
                inputs, targets = inputs.to(self.device), targets.to(self.device)
...
def train_shadow_model(shadow_path, device, shadow_model, train_loader, test_loader, use_DP, noise, norm, loss, optimizer, delta):
...
# black shadow
def attack_mode0(TARGET_PATH, SHADOW_PATH, ATTACK_PATH, device, use_DP, attack_trainloader, attack_testloader, 
    ...
    attack = attack_for_blackbox(SHADOW_PATH, TARGET_PATH, ATTACK_SETS, 
                                attack_trainloader, attack_testloader, target_model, shadow_model, attack_model, device, use_DP)
...
def attack_mode1(TARGET_PATH, ATTACK_PATH, device, use_DP, attack_trainloader, attack_testloader, 
    ...
    attack = attack_for_blackbox(TARGET_PATH, TARGET_PATH, ATTACK_SETS, attack_trainloader, attack_testloader, target_model, target_model, attack_model, device, use_DP)
...
# white shadow
def attack_mode3(TARGET_PATH, SHADOW_PATH, ATTACK_PATH, device, use_DP, attack_trainloader, attack_testloader, 
                    target_model, shadow_model, attack_model, get_attack_set, num_classes):
    ...
    attack = attack_for_whitebox(TARGET_PATH, SHADOW_PATH, ATTACK_SETS, attack_trainloader, attack_testloader, 
                                    target_model, shadow_model, attack_model, device, use_DP, num_classes)
