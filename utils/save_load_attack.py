...
def save_defense_result(
    ...
    logging.info(f"saving...")
    logging.debug(f"location : {save_path}/defense_result.pth") #, content summary :{pformat(summary_dict(save_dict))}")

    torch.save(
        save_dict,
        f'{save_path}/defense_result.pth',
    )
...
def load_attack_result(
    ...
    if all(key in load_file for key in [
        ...
        print("--------------clean_setting.dataset_path------------", clean_setting.dataset_path)
        ...
        print("***********************************************************")
        import pprint
        pprint.pprint(vars(clean_setting))
