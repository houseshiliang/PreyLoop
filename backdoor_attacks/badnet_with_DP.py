...
# public parameters for class badnet and its inherited classes(ssba)
...
class BadNet(NormalCase):
    ...
    def set_bd_args(cls, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        # bd_yaml_path 融合到 yaml_path 中
        # parser.add_argument('--bd_yaml_path', type=str, default='../backdoor_resource/config/attack/badnet/default.yaml',
    ...
    def stage1_non_training_data_prepare(self):
        ...
        ###1、替换数据生成方式，以及正常数据集的transform
        
        ### train/test_label_transform are None
        ...
        ### get the backdoor transform on img
        train_bd_img_transform, test_bd_img_transform = bd_attack_img_trans_generate(args)

        images=np.load("../data/datasets/fmnist_seperate_images/train_encode.npy")
        print(images.shape)

        # print("*****************************", args.attack_train_replace_imgs_path)
        # print("------------00000----------------", args.img_size[:2])   # 实际的输入大小
        ### get the backdoor transform on label
        bd_label_transform = bd_attack_label_trans_generate(args)

        ### **************************** prepare backdoor train data ****************************
        
        ### set the train-poison-index of the backdoor attack data
        ...
        ### **************************** prepare backdoor test data ****************************
        ### decide which img to poison in ASR (attack success rate) Test
    ...
    def stage2_training(self):
        ...
        if not ModuleValidator.is_valid(self.net):
        ...
        # # -------------------------------- change for DP-based training --------------------------------
        # self.privacy_engine = PrivacyEngine(accountant=args.DP_type)  # "rdp"

        # trainloader = DataLoader(bd_train_dataset_with_transform, batch_size=args.batch_size, shuffle=True, drop_last=True,
        #                pin_memory=args.pin_memory, num_workers=args.num_workers)

        # self.noise_multiplier, self.max_grad_norm = args.noise, args.norm
        ...
        print ("************ args.dataset_path1 ***************", args.dataset_path)
